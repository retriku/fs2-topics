...
class AckerConsumerWrongOrderDemo[F[_]: ConcurrentEffect](fs2Rabbit: RabbitClient[F]) {
...
class WrongOrderFlow[F[_]: ConcurrentEffect](
    ...
    for {
      ...
      _       <-
        consumer
          ...
          .evalTap(i =>
            prev.get.flatMap { p =>
              if ((i.payload - p) != 1 && (i.payload > 0 || p > 0)) putStrLnThread(s"Expected ${p + 1} got ${i.payload}")
              else ().pure[F]
            })
